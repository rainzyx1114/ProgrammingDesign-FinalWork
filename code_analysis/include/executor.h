...
private:
    ...
    VariableInfo buildReturnValueInfo(const Value v);
    std::string inferValueType(const Value& value) const;
    bool isAutoGenerated(const std::string& name) const;
    ObjectView buildObjectViewFromObject(const std::string& objectId,
                                         const std::shared_ptr<Object>& obj) const;

    // "this" context for method execution (for member variable access and virtual dispatch)
    std::string currentClassName;
    std::shared_ptr<Object> currentThis;
    ...
    void recordSnapshot(const std::string& event, int lineNumber = 0, const Value* returnedValue = nullptr);
